...
const truncateBody = (length: number, body: string): string =>
  `${body.slice(0, length)}...`;
    ...
    >
      <div className="max-w-sm rounded overflow-hidden shadow-md m-2 flex flex-col bg-white">
        <div className={statusColor(props.entry.published)}>
          <p className="px-6 py-1 font-bold">
            {props.entry.published ? "公開中" : "非公開"}
          </p>
        </div>
        <div className="px-6 py-4">
          <h1 className="font-bold text-xl mb-3 text-grey-darkest">
            {props.entry.title}
          </h1>
